...
!   vortex_fixed_imp_mpi_filter_secondary.f90 => Simulates a fixed number of vortices
...
!                                      uses a secondary sinusoidal perturbation
!----------------------------------------------------------------------
...
!    initialize the vortex position
    do i=1,n
    si(i)=(i-1)*delx
    z(i)=cmplx(si(i)+0.01q0*sin(kw*si(i)),-0.01q0*sin(kw*si(i))+0.0005q0*sin(4.0q0*kw*si(i)),16)
    enddo
...
!    krasny filter starts here
    ...
    if(j .eq. 0) then
        tag=0
    open(unit=38,file='/gpfs/bglscratch/aserajsh/fixed_vortex/filter_status',position='append',status='old')
